function configure_log(){
    ...
    do
        iniset $f DEFAULT ogging_exception_prefix ''
        iniset $f DEFAULT ogging_debug_format_suffix 'from (pid=%(process)d) %(funcName)s %(pathname)s:%(lineno)d'
        iniset $f DEFAULT ogging_default_format_string '%(asctime)s.%(msecs)03d %(process)d %(levelname)s %(name)s %(instance)s%(message)s'
        iniset $f DEFAULT ogging_context_format_string '%(asctime)s.%(msecs)03d %(process)d %(levelname)s %(name)s [%(request_id)s %(project_id)s %(user_id)s] %(instance)s%(message)s'
    done
